---
name: knack-auth
description: Use when authenticating against the Knack API for HTI — fetching API keys, creating user sessions via `POST /v1/applications/{app_id}/session`, refreshing 24-hour user tokens before expiry, or validating credentials before any read/write. Required prelude for knack-reader, knack-pagination, knack-exporter, knack-reporting-sync. Trigger on any "401 from Knack", "Knack token expired", "rotate Knack API key", "log in as admin", or pre-Knack-operation auth bootstrap.
allowed-tools: Read
---
