...
description: Use when building dynamic filters, sorts, or optimized queries for HTI Knack dashboards and compliance reports. Reduces API payload size, respects the 10-req/sec rate limit, and improves dashboard load time. Trigger on any "filter the Knack records", "sort by X", or slow-dashboard query-optimization task.
